...
const EmailComposer = () => {
  const [registry, setRegistry] = useState({ Invoices: [], Quotations: [], Receipts: [], Services: [], Clients: [] });
  ...
  // --- ENHANCED COMMUNICATION: ERROR HANDLER ---
  const handleSystemError = useCallback((error, fallbackMessage) => {
    ...
    if (status === 401 || message === "SESSION_EXPIRED") {
      ...
      toast.error("ACCESS_FORBIDDEN: Administrative clearance required for dispatch.", {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(`${BASE_URL}/finance/registry?limit=1000`, {
          method: "GET",
          headers: {
        ...
        if (!response.ok) throw new Error("REGISTRY_LINK_FAILED");
        
        const data = await response.json();
        setRegistry(data);
      } catch (err) {
        handleSystemError(err, "DATABASE_OFFLINE");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleDocSelect = (doc) => {
    ...
    if (type === 'Service') body = `Dear ${clientName},\n\nPlease find the service valuation and completion report attached.`;
    
    setMessage(`${body}\n\nOur team is available should you require further clarification.\n\nRegards,\nFinance Operations\nSMA Systems`);
    toast.success("SOURCE_OBJECT_LOADED", { duration: 1000 });
  };

  // 4. DISPATCH ENGINE (WITH ENHANCED COMMUNICATION)
  const handleSend = async () => {
    ...
    // CONFIRMATION TOAST FOR BULK DISPATCH
    if (mode === "bulk") {
        toast((t) => (
            <div className="flex flex-col gap-2">
              ...
              <div className="flex gap-2">
                <button 
                  onClick={() => { toast.dismiss(t.id); executeDispatch(); }}
                  className="bg-emerald-500 px-3 py-1 rounded text-[8px] font-black uppercase text-slate-900"
                >Execute</button>
  ...
  const executeDispatch = async () => {
    const loadingToast = toast.loading("INITIALIZING_SECURE_SMTP_RELAY...");
    setStatus('sending');
    ...
    try {
      ...
      if (!response.ok) {
        ...
        const err = new Error(result.error || result.message || "GATEWAY_REJECTION");
        err.status = response.status;
        throw err;
      }

      toast.success("DISPATCH_SEQUENCE_COMPLETE", { id: loadingToast });
      setStatus('success');
      setAttachedFile(null);
    } catch (err) {
    ...
    <div className="h-screen w-full bg-[#0F172A] flex flex-col overflow-hidden text-slate-300 font-sans text-[10px] antialiased">
      ...
      <header className="h-14 border-b border-slate-800 bg-[#0F172A] flex items-center justify-between px-6 shrink-0 z-30">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2 pr-6 border-r border-slate-800">
            <div className="w-6 h-6 bg-emerald-500 rounded flex items-center justify-center text-slate-900 shadow-lg">
          ...
          <div className="p-4 border-b border-slate-800/50 space-y-3">
            <div className="relative">
              ...
              <input 
                ...
                className="w-full bg-slate-950 border border-slate-800 rounded-md py-2 pl-9 pr-3 text-[9px] font-bold text-white outline-none focus:border-emerald-500/30 transition-all"
              />
            </div>
                ...
                <button key={doc._id} onClick={() => handleDocSelect(doc)} className={`w-full text-left p-3 rounded-lg transition-all group border ${selectedDoc?._id === doc._id ? "bg-slate-800/80 border-emerald-500/50" : "hover:bg-slate-900 border-transparent"}`}>
                  <div className="flex items-center justify-between">
                    <div className="space-y-1 overflow-hidden">
                      <p className={`text-[10px] font-black uppercase truncate ${selectedDoc?._id === doc._id ? "text-emerald-400" : "text-slate-300"}`}>
                        {doc.client?.name || doc.name || "Unknown_Entity"}
                      </p>
              ...
              <div className="bg-white border border-slate-200 shadow-2xl shadow-slate-200/50 rounded-xl overflow-hidden">
                ...
                <div className="p-8 space-y-6">
                  ...
                  <div 
                    ...
                    className={`p-4 rounded-xl flex items-center justify-between group cursor-pointer transition-all border-2 border-dashed ${attachedFile ? "bg-emerald-50 border-emerald-200" : "bg-slate-50 border-slate-200 hover:border-indigo-300"}`}
                  >
                    <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-sm ${attachedFile ? "bg-emerald-500 text-white" : "bg-white text-slate-400"}`}>
                        <Paperclip size={16}/>
                      </div>
                      <div>
                        <p className={`text-[9px] font-black uppercase tracking-widest ${attachedFile ? "text-emerald-700" : "text-slate-900"}`}>
                          {attachedFile ? attachedFile.name : (mode === "single" ? `${activeTab.slice(0,-1).toUpperCase()}_${selectedDoc._id.slice(-6).toUpperCase()}.pdf` : "CONSOLIDATED_BATCH_ARCHIVE.ZIP")}
                        </p>
      ...
      {status === 'success' && (
        <div className="fixed inset-0 bg-slate-950/95 backdrop-blur-md z-[100] flex flex-col items-center justify-center animate-in fade-in duration-500">
          <div className="w-24 h-24 bg-emerald-500 text-slate-950 rounded-full flex items-center justify-center mb-8 shadow-[0_0_50px_rgba(16,185,129,0.4)] animate-bounce">
