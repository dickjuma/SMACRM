...
import {
  ...
  ClipboardList, Package, Target, ShieldCheck, Bot,
...
// ============================
// API CONFIGURATION - UPDATED TO MATCH YOUR ROUTER
// ============================
...
// Finance API calls - UPDATED ENDPOINTS
const financeApi = {
  // Get all documents (invoices, quotations, receipts, services) and clients
  getRegistry: () => api.get('/email/registry/data'), // Changed from /dispatch/registry/data
  
  // Individual document endpoints (keep as is if they exist in your backend)
...
// Email API calls - UPDATED ENDPOINTS
const emailApi = {
...
// Dispatch API calls - UPDATED TO MATCH YOUR ROUTER ENDPOINTS
const dispatchApi = {
  // Send email - matches your router POST /send
  ...
  // Email statistics - matches your router GET /stats
  getEmailStats: (period = '30d') => api.get('/email/stats', { params: { period } }),
  
  // Email history - matches your router GET /history
  ...
  // Get specific email - matches your router GET /:id
  getEmail: (id) => api.get(`/email/${id}`),
  
  // Resend email - matches your router POST /:id/resend
  resendEmail: (id) => api.post(`/email/${id}/resend`),
  
  // Delete email - matches your router DELETE /:id
  deleteEmail: (id) => api.delete(`/email/${id}`),
  
  // Bulk clients for email - matches your router GET /clients/bulk
  getBulkClients: (params) => api.get('/email/clients/bulk', { params }),
  
  // Apply template - matches your router POST /template/apply
  applyTemplate: (data) => api.post('/email/template/apply', data),
  
  // Test email - matches your router POST /test
  testEmail: () => api.post('/email/test'),
  
  // Health check - matches your router GET /health
  healthCheck: () => api.get('/email/health'),
  
  // Bulk operations - matches your router POST /bulk/operations
  bulkOperations: (data) => api.post('/email/bulk/operations', data),
  
  // Analytics - matches your router GET /analytics/overview
...
const StatisticsPanel = ({ data }) => {
  ...
  const { data: emailStats, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await dispatchApi.getEmailStats('30d');
...
// Enhanced AI Assistant with multiple features
const AIAssistant = ({ onSuggestion, message, onImprove }) => {
  ...
  const handleSuggestion = async (type) => {
    setIsLoading(true);
    const suggestions = {
      greeting: "I hope this email finds you well. I'm writing to follow up on your recent inquiry.",
      professional: "After careful consideration and analysis of your requirements, we are pleased to present...",
      urgent: "This matter requires your immediate attention to ensure continued service without interruption.",
      closing: "We appreciate your prompt attention to this matter and look forward to your response.",
      followup: "Following up on our previous correspondence, we wanted to ensure you received all necessary information.",
      thankYou: "Thank you for your continued partnership and trust in our services.",
      reminder: "This serves as a gentle reminder regarding the upcoming deadline for your consideration."
    };
  ...
  const handleAIOperation = async () => {
    if (!message && !prompt) {
      toast.error("Please provide content or a prompt");
      return;
    }

    setIsLoading(true);
    setTimeout(() => {
      let improvedText = message;
      
      switch(aiMode) {
        case "improve":
          improvedText = `I hope this email finds you well.\n\n${message}\n\nWe appreciate your attention to this matter and look forward to your response.\n\nBest regards,\n[Your Company]`;
          break;
        case "shorten":
          improvedText = message.split('\n').slice(0, 5).join('\n');
          break;
        case "formalize":
          improvedText = message.replace(/Hi/g, "Dear").replace(/Thanks/g, "Thank you").replace(/Cheers/g, "Sincerely");
          break;
        case "translate":
          improvedText = `${message}\n\n[Spanish Translation]\n${message}`;
          break;
        default:
          break;
      }
      ...
      setIsLoading(false);
      toast.success(`AI ${aiMode} completed!`);
    }, 1200);
  };
    ...
    <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-gray-800 uppercase tracking-wider">AI Assistant</h3>
        </div>
        <div className="flex items-center gap-2">
          <span className="px-2 py-1 bg-blue-100 text-blue-700 text-xs font-bold rounded-lg">PRO</span>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {["improve", "shorten", "formalize", "translate"].map((mode) => (
              <button
                key={mode}
        ...
        <div>
          <label className="text-xs font-medium text-gray-700 mb-1 block">Custom Instruction</label>
          <div className="relative">
            <input
              type="text"
              placeholder="e.g., 'Make this more professional' or 'Translate to Spanish'"
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              className="w-full bg-white border border-gray-200 rounded-lg px-3 py-2 pl-10 text-sm text-gray-800 placeholder-gray-400 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 outline-none"
            />
            <Wand2 size={16} className="absolute left-3 top-2.5 text-gray-400" />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {["greeting", "professional", "urgent", "closing", "thankYou", "reminder"].map((type) => (
              <button
                key={type}
                onClick={() => handleSuggestion(type)}
                disabled={isLoading}
                className="px-3 py-1.5 bg-gray-100 border border-gray-200 hover:border-gray-300 hover:bg-gray-200 text-gray-700 rounded-lg text-xs font-medium transition-colors capitalize disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isLoading ? "..." : type.replace(/([A-Z])/g, ' $1').trim()}
              </button>
            ))}
          </div>
        </div>
        ...
        <button
          ...
          disabled={isLoading || (!message && !prompt)}
          className="w-full bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-700 hover:to-blue-600 text-white py-2.5 rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2 shadow-sm disabled:opacity-50"
        >
          {isLoading ? (
          ...
          {isLoading ? "Processing..." : `Apply ${aiMode}`}
        </button>
      </div>
    </div>
  );
        ...
        <div className="flex items-center justify-between">
          ...
          <label className="relative inline-flex items-center cursor-pointer">
            <input 
              ...
              onChange={() => setOpenTracking(!openTracking)}
        ...
        <div className="flex items-center justify-between">
          ...
          <label className="relative inline-flex items-center cursor-pointer">
            <input 
              ...
              onChange={() => setClickTracking(!clickTracking)}
              className="sr-only peer" 
            />
            <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-blue-600"></div>
          </label>
...
const EmailHistory = () => {
  ...
  const { data: emailHistory = [], isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      try {
        const response = await dispatchApi.getEmailHistory({ filter });
    ...
    <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
      ...
      <div className="space-y-2">
        ...
        >
          ...
          {signatures.length === 0 && (
            <option value={0}>No signatures available</option>
          )}
        </select>
        <button 
          onClick={handleApplySignature}
          disabled={signatures.length === 0}
          className="w-full flex items-center justify-between px-3 py-2.5 bg-blue-50 hover:bg-blue-100 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-blue-50 rounded-lg text-sm font-medium text-blue-700 transition-colors border border-blue-200"
        >
          ...
          {signatures.length > 0 && (
            <span className="text-xs text-blue-500">
              {signatures[activeSignature]?.name}
            </span>
          )}
        </button>
      </div>
...
const EmailComposerContent = () => {
  ...
  const [ccRecipients, setCcRecipients] = useState(["accounts@company.com"]);
        ...
        const mockInvoices = [
          {
            ...
            client: { name: "Acme Corp", email: "billing@acmecorp.com" },
            amount: 12500.50,
            total: 12500.50,
            status: "pending",
            dueDate: "2023-12-15",
          ...
          },
          {
            ...
            client: { name: "Global Tech", email: "finance@globaltech.com" },
            amount: 8900.00,
            total: 8900.00,
            status: "paid",
            dueDate: "2023-11-30",
  ...
  // Mutation for sending email - UPDATED FOR CORRECT ENDPOINTS
  const dispatchMutation = useMutation({
    mutationFn: async (formData) => {
      ...
      if (formData.documents && formData.documents.length > 0) {
        formDataObj.append('documents', JSON.stringify(formData.documents.map(d => d._id)));
      }
      ...
      if (formData.mode === "single") {
        ...
        if (formData.recipient) {
          formDataObj.append('recipient', formData.recipient);
        }
        if (formData.document) {
          formDataObj.append('document', JSON.stringify(formData.document));
        }
      } else {
        // Bulk mode
      ...
      // Call real API - USING UPDATED ENDPOINT
      const response = await dispatchApi.dispatchEmail(formDataObj);
      return response;
    },
    onMutate: () => {
      setProgress(0);
  ...
  const applyTemplate = async (tempKey, doc = selectedDoc) => {
    ...
    try {
      ...
      const response = await dispatchApi.applyTemplate(templateData);
      ...
      if (response.data) {
        ...
        toast.success(`"${TEMPLATES[tempKey].label}" template applied!`, {
          icon: '📧',
          duration: 2000
        });
      }
  ...
  const handleDocSelect = (doc) => {
    ...
    toast.success(`Selected ${doc.client?.name || doc.clientDetails?.name || 'document'}`, {
      duration: 1500
    });
  };
  ...
  const handleDispatch = () => {
    ...
    const formData = {
      ...
      tracking,
      file: attachedFile,
      documents: attachedDocuments.map(d => d._id)
    };
  ...
  const handleInsertVariable = (variable) => {
    ...
    setMessage(prev => 
      prev.substring(0, start) + variable + prev.substring(end)
    );
  ...
  const handleClearAll = () => {
    ...
    setSelectedDoc(null);
    setSelectedClients([]);
    setCcRecipients(["accounts@company.com"]);
    setBccRecipients([]);
      ...
      <header className="h-16 border-b border-gray-200 bg-white flex items-center justify-between px-6 shrink-0 z-50 shadow-sm">
        <div className="flex items-center gap-4">
          <button 
            ...
            className="p-2 hover:bg-gray-100 rounded-lg transition-all lg:hidden text-gray-600 hover:text-gray-800"
          >
            <Menu size={20} />
          </button>
          ...
          <div className="flex bg-gray-100 p-1 rounded-lg border border-gray-200 ml-4">
            {["single", "bulk"].map(m => (
              <button 
                ...
                className={`px-4 py-2 rounded-md text-xs font-medium uppercase transition-all ${
                  mode === m 
        ...
        <div className="flex items-center gap-3">
          ...
          {!dispatchMutation.isSuccess && (
            ...
            >
              ...
              <span>
                {mode === 'bulk' ? `Send (${selectedClients.length})` : 'Send Now'}
              </span>
            </button>
          )}
        </div>
          ...
          <div className="p-5 space-y-6">
            <div className="relative group">
              <input 
                ...
                className="w-full bg-gray-50 border border-gray-200 rounded-lg py-3 px-10 text-sm text-gray-800 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all placeholder-gray-400"
              />
              ...
              <div className="flex items-center justify-between px-2">
                ...
                >
                  {selectedClients.length === filteredClients.length ? 
                    <CheckSquare size={18} className="text-blue-600"/> : <Square size={18} className="text-gray-400"/>
                  }
                  Select All
                </button>
              ...
              <div className="grid grid-cols-2 gap-2 bg-gray-50 p-2 rounded-lg border border-gray-200">
                {["Invoices", "Quotations", "Receipts", "Services"].map(t => (
                  <button 
                    ...
                    className={`py-2.5 rounded-md text-xs font-medium transition-all flex items-center justify-center gap-2 ${
                      activeTab === t 
                        ? "bg-blue-600 text-white shadow-sm" 
                        : "text-gray-600 hover:text-gray-800 hover:bg-gray-200"
                    }`}
        ...
        <main className="flex-1 bg-white overflow-y-auto relative">
          ...
          {(selectedDoc || (mode === "bulk" && selectedClients.length > 0)) && (
            <button 
              ...
              className="lg:hidden fixed top-20 left-4 z-30 flex items-center gap-2 bg-white px-4 py-2 rounded-lg border border-gray-200 text-gray-700 shadow-sm font-medium text-sm hover:bg-gray-50 transition-all group"
            >
              ...
              {dispatchMutation.isSuccess ? (
                <div className="bg-white border border-gray-200 rounded-xl p-8 md:p-12 text-center shadow-sm">
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 justify-center mt-8">
                    <button 
                      onClick={() => { dispatchMutation.reset(); setSelectedDoc(null); setSelectedClients([]); }}
                    ...
                    <button 
                      ...
                      className="px-6 py-3 border border-gray-200 text-gray-700 rounded-lg font-medium text-sm hover:bg-gray-50 transition-colors"
                    >
                      Print Confirmation
                    </button>
                  </div>
                ...
                <div className="space-y-6">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {Object.entries(TEMPLATES).map(([key, t]) => (
                        <button 
                          ...
                          className={`flex items-center gap-2 px-4 py-3 rounded-lg border text-sm font-medium transition-all hover:scale-[1.02] ${
                            activeTemplate === key 
                              ? `${t.borderColor} ${t.bgColor} border-2 text-gray-800 shadow-sm` 
                  ...
                  <div className="grid lg:grid-cols-3 gap-6">
                    ...
                    <div className="space-y-6">
                      <AIAssistant 
                        onSuggestion={(suggestion) => setMessage(prev => prev + "\n\n" + suggestion)}
                        message={message}
                        onImprove={(improved) => setMessage(improved)}
                      />
                        ...
                        <div className="p-5 md:p-6 space-y-6">
                                  ...
                                  <div className="flex-1 flex flex-wrap gap-2">
                                    {bccRecipients.map((email, idx) => (
                                      <div key={idx} className="flex items-center gap-1 bg-white px-2 py-1 rounded border border-gray-200">
                                        <span className="text-sm text-gray-800">{email}</span>
                                        <button 
                                          onClick={() => removeBccRecipient(idx)}
                                          className="text-gray-400 hover:text-red-500"
                                        >
                                          <X size={14} />
                                        </button>
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="relative">
                              <input 
                                ...
                                maxLength={120}
                                className="w-full bg-white border border-gray-300 px-4 py-3 rounded-lg text-base font-medium text-gray-800 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all placeholder-gray-400"
                          ...
                          <div className="space-y-3">
                            ...
                            <textarea 
                              ...
                              className="w-full bg-white border border-gray-300 p-4 rounded-lg text-sm font-normal text-gray-700 outline-none h-64 resize-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all leading-relaxed font-sans shadow-inner"
                          ...
                          {(attachedFile || attachedDocuments.length > 0) && (
                            <div className="border border-gray-200 rounded-lg p-4">
                              <div className="flex items-center justify-between mb-3">
                                ...
                                <span className="text-xs text-gray-500">
                                  {attachedDocuments.length + (attachedFile ? 1 : 0)} file(s)
                                </span>
                              </div>
                              <div className="space-y-2">
                                {attachedFile && (
                                  <div className="flex items-center justify-between p-2 bg-gray-50 rounded-lg">
                                    ...
                                    <button 
                                      onClick={() => handleRemoveAttachment(0, true)}
                          ...
                          <div 
                            ...
                            className={`p-5 rounded-lg border-2 border-dashed flex items-center justify-between cursor-pointer transition-all ${
                              attachedFile 
                            ...
                            <div className="flex items-center gap-4">
                              ...
                              <div className="overflow-hidden">
                                ...
                                <p className="text-xs text-gray-500 mt-1">
                                  {attachedFile 
                                    ? `${(attachedFile.size / 1024).toFixed(1)} KB · Click to change`
                                    : 'Click to attach a file (PDF, DOC, IMG, etc.)'
                                  }
                                </p>
                              </div>
                            </div>
                          ...
                          <div className="flex flex-wrap gap-3 pt-4">
                            ...
                            <button
                              onClick={handleDispatch}
                              disabled={dispatchMutation.isPending || (mode === 'single' && !selectedDoc) || (mode === 'bulk' && selectedClients.length === 0)}
                              className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition-colors shadow-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                            >
                              {dispatchMutation.isPending ? (
                                <>
                                  <Loader2 size={16} className="animate-spin" />
            ...
            <div className="h-full flex flex-col items-center justify-center p-8 text-center">
              ...
              <div className="bg-gray-50 rounded-lg p-4 max-w-md">
                ...
                <ul className="text-xs text-gray-600 space-y-1 text-left">
                  ...
                  <li className="flex items-center gap-2">
                    <Check size={12} className="text-emerald-500" />
                    AI Assistant helps refine and optimize your content
                  </li>
                  <li className="flex items-center gap-2">
                    <Check size={12} className="text-emerald-500" />
                    All sent emails are tracked in history
                  </li>
                  <li className="flex items-center gap-2">
                    <Check size={12} className="text-emerald-500" />
                    Attach files and documents easily
                  </li>
                </ul>
              </div>
            </div>
