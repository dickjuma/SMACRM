...
import { 
  ...
  CheckSquare, Square, Users, Search // Added Search import
} from "lucide-react";
...
const EmailComposerContent = () => {
  ...
  // --- TANSTACK MUTATION: DISPATCH ENGINE ---
  ...
  const toggleClient = (email) => {
    if (!email) {
      toast.error("Client has no valid email node");
      return;
    }
  ...
  const handleDispatch = () => {
    if (mode === 'bulk' && selectedClients.length === 0) {
      return toast.error("SELECT_RECIPIENTS_FIRST");
    }
  ...
  return (
    <div className="h-screen w-full bg-[#0F172A] flex flex-col overflow-hidden text-slate-300 font-sans text-[10px] antialiased">
      ...
      <header className="h-16 border-b border-slate-800 bg-[#0F172A]/80 backdrop-blur-md flex items-center justify-between px-6 shrink-0 z-50">
        <div className="flex items-center gap-6">
          <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 hover:bg-slate-800 rounded-lg lg:hidden"><Menu size={18} /></button>
          ...
          <div className="flex bg-slate-950 p-1 rounded-xl border border-slate-800">
            {["single", "bulk"].map(m => (
              <button key={m} onClick={() => { setMode(m); setSelectedDoc(null); mutation.reset(); }} className={`px-5 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all ${mode === m ? "bg-slate-800 text-emerald-400" : "text-slate-600"}`}>{m}</button>
            ))}
          </div>
        </div>

        {!mutation.isSuccess && (
          <button onClick={handleDispatch} disabled={mutation.isPending || (!selectedDoc && mode === 'single')} className="flex items-center gap-3 bg-emerald-500 hover:bg-emerald-400 text-slate-950 px-8 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all shadow-xl shadow-emerald-500/20 active:scale-95 disabled:opacity-20">
            {mutation.isPending ? <Loader2 size={14} className="animate-spin"/> : <Send size={14} strokeWidth={3}/>}
            {mode === 'bulk' ? `Dispatch (${selectedClients.length})` : 'Authorize'}
          </button>
        )}
      </header>
        ...
        <aside className={`fixed lg:static inset-y-0 left-0 z-40 w-72 bg-[#0F172A] border-r border-slate-800 flex flex-col transition-transform duration-300 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-3 space-y-1.5">
            {mode === "single" ? filteredDocuments.map(doc => (
              <button key={doc._id} onClick={() => handleDocSelect(doc)} className={`w-full text-left p-4 rounded-2xl border transition-all ${selectedDoc?._id === doc._id ? "bg-emerald-500/10 border-emerald-500/30" : "border-transparent hover:bg-slate-900"}`}>
                <p className="text-[10px] font-black uppercase truncate text-slate-300">{doc.client?.name || "Entity"}</p>
                <p className="text-[8px] font-mono text-slate-600 mt-1">#{doc._id?.slice(-8).toUpperCase()}</p>
              </button>
        ...
        <main className="flex-1 bg-[#F8FAFC] overflow-y-auto relative custom-scrollbar-light">
          {(selectedDoc || mode === "bulk") ? (
            <div className="max-w-4xl mx-auto p-6 md:p-12 relative">
              ...
              {mutation.isSuccess ? (
                <div className="bg-white border border-slate-200 rounded-[2.5rem] p-16 text-center shadow-2xl animate-in zoom-in-95 duration-500">
                  <div className="w-20 h-20 bg-emerald-500 text-white rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-xl shadow-emerald-200 animate-bounce">
                    <Check size={40} strokeWidth={3} />
                  </div>
                  <h2 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">Relay Success</h2>
                  <button onClick={() => mutation.reset()} className="mt-10 flex items-center gap-2 bg-slate-900 text-white px-8 py-3 rounded-2xl font-black text-[10px] uppercase mx-auto hover:bg-emerald-600 transition-all">
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-white border border-slate-200 shadow-2xl rounded-[2.5rem] overflow-hidden relative">
                    {mutation.isPending && (
                    ...
                    <div className="p-8 md:p-10 space-y-8">
                      <div className="space-y-2">
                        <label className="text-[9px] font-black uppercase text-slate-400 ml-1">Recipient_Node</label>
                        <div className="flex items-center gap-3 bg-slate-900 px-6 py-4 rounded-2xl border border-slate-800 shadow-inner">
                      ...
                      <div className="space-y-2">
                        ...
                        <input value={subject} onChange={(e) => setSubject(e.target.value)} className="w-full bg-slate-50 border border-slate-200 px-6 py-4 rounded-2xl text-[12px] font-black text-slate-700 outline-none focus:border-indigo-500 transition-all" />
                      </div>
                      <div className="space-y-2">
                        <label className="text-[9px] font-black uppercase text-slate-400 ml-1">Content</label>
                        <textarea value={message} onChange={(e) => setMessage(e.target.value)} className="w-full bg-slate-50 border border-slate-200 p-8 rounded-[2rem] text-[12px] font-medium text-slate-600 outline-none h-64 resize-none focus:border-indigo-500 transition-all leading-relaxed" />
                      </div>

                      <div onClick={() => fileInputRef.current.click()} className={`p-6 rounded-[2rem] border-2 border-dashed flex items-center justify-between cursor-pointer transition-all ${attachedFile ? "bg-emerald-50 border-emerald-200" : "bg-slate-50 border-slate-200 hover:border-indigo-300"}`}>
                        <div className="flex items-center gap-5">
                          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${attachedFile ? "bg-emerald-500 text-white" : "bg-white text-slate-400 shadow-sm"}`}><Paperclip size={18}/></div>
                          <div>
                            <p className="text-[10px] font-black text-slate-900 uppercase truncate max-w-[200px]">{attachedFile ? attachedFile.name : 'Generated_Payload.pdf'}</p>
                            <p className="text-[8px] font-bold text-slate-400 uppercase mt-0.5">Attachment_Status</p>
                          </div>
                        </div>
                        {attachedFile && <X size={16} className="text-red-500" onClick={(e) => {e.stopPropagation(); setAttachedFile(null);}}/>}
                      </div>
                    </div>
                  </div>
                </div>
